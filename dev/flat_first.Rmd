...
# datafile <- system.file("CWP_dataset.csv", package = "CWP.dataset")
...
```{r test-filtering_function}
# Ajouter les tests pour filtering_function
```


```{r function-last_path_reduced}
...
```{r test-last_path_reduced}
# No test

```
# Functions for reporting 

```{r function-knitting_plots_subfigures}
...
```{r test-knitting_plots_subfigures}
# No test
```

# render_subfigures

```{r function-render_subfigures}
...
```{r test-bar_plot_default}
# Pas de test pour cette fonction
```

## generate_plot

```{r function-generate_plot}
#' Documentation to do 
#' @name generate_plot
...
```{r test-generate_plot}
# Pas de test pour cette fonction
```

## pie_chart_2_default

```{r function-pie_chart_2_default}
...
```{r test-pie_chart_2_default}
# Pas de test pour cette fonction
```

## fonction_empreinte_spatiale

```{r function-fonction_empreinte_spatiale}
...
```{r test-fonction_empreinte_spatiale}
# Pas de test pour cette fonction
```

## save_image

```{r function-save_image}
#' Save Plot as Image
#'
...
```{r function-function_multiple_comparison}
...
```{r test-function_multiple_comparison, eval=FALSE}
...
```{r function-compute_summary_of_differences}
...
```{r test-fonction_groupement}
# No test
```

#### Function: separate_chunks_and_text

```{r function-separate_chunks_and_text}
...
```{r test-separate_chunks_and_text}
...
fusen::inflate(flat_file = "dev/flat_first.Rmd", vignette_name = "First",overwrite = "yes")
```
