...
```{r AddLines, eval=exists("Add_lines"), results='asis', echo=FALSE}
x <- Add_lines

# 1) Si c'est un chemin vers un fichier existant -> knit_child
if (is.character(x) && length(x) == 1 && file.exists(x)) {
  res <- knitr::knit_child(x, quiet = TRUE, envir = environment())
  cat(res)

# 2) Si c'est du texte (string unique) -> cat direct
} else if (is.character(x) && length(x) == 1 && nzchar(x)) {
  cat(x)

# 3) Si c'est un vecteur de lignes -> cat en joignant
