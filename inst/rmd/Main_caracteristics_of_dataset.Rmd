...
`r if(exists("other_dimension_analysis_list", envir =environment()) && !is.null(other_dimension_analysis_list)){CWP.dataset::cat_title("## Coverage for other dimensions", child_headerinside = child_header)}}else {""}`

`r if(exists("other_dimension_analysis_list", envir =environment()) && !is.null(other_dimension_analysis_list)){cat("We check the distribution of the value of each dimension and each unit.)}}else {""}`
